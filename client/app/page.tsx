...
function page() {
  ...
  return (
    <div className='flex flex-col items-center'>
      <Navbar />
      <div className='w-[70vw] h-[75vh] flex flex-col'>
        {messages.map((item, index)=>{
          if(item.role == 'user'){ 
            ...
            return <div key={index} className='flex justify-start mt-2'>🤖<div className='p-2 bg-lime-950 text-white rounded-lg max-w-[85%]'><h2>{item.content}</h2></div></div>
          }
        })}
      </div>
